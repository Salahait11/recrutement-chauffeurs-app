...
<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 dark:text-gray-200 leading-tight">
            {{ __('Modifier le Type de Congé :') }} {{ $leaveType->name }}
        </h2>
    </x-slot>
            ...
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900 dark:text-gray-100">
                    ...
                    <form method="POST" action="{{ route('admin.leave-types.update', $leaveType->id) }}">
                        @csrf
                        @method('PUT') {{-- Important pour la modification --}}
                            ...
                            <div class="grid grid-cols-1 sm:grid-cols-3 gap-6">
                                <div>
                                    ...
                                    <select name="requires_approval" id="requires_approval" class="block mt-1 w-full rounded-md shadow-sm border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300 focus:border-indigo-500 dark:focus:border-indigo-600 focus:ring-indigo-500 dark:focus:ring-indigo-600" required>
                                         {{-- Comparaison avec 1/0 car la valeur vient de la DB --}}
                                        <option value="1" {{ old('requires_approval', $leaveType->requires_approval) == 1 ? 'selected' : '' }}>Oui</option>
                                        <option value="0" {{ old('requires_approval', $leaveType->requires_approval) == 0 ? 'selected' : '' }}>Non</option>
                                    </select>
                                 ...
                                 <div>
                                    ...
                                    <select name="affects_balance" id="affects_balance" class="block mt-1 w-full rounded-md shadow-sm border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300 focus:border-indigo-500 dark:focus:border-indigo-600 focus:ring-indigo-500 dark:focus:ring-indigo-600" required>
                                        <option value="1" {{ old('affects_balance', $leaveType->affects_balance) == 1 ? 'selected' : '' }}>Oui</option>
                                        <option value="0" {{ old('affects_balance', $leaveType->affects_balance) == 0 ? 'selected' : '' }}>Non</option>
                                    </select>
                                 ...
                                 <div>
                                    ...
                                    <select name="is_active" id="is_active" class="block mt-1 w-full rounded-md shadow-sm border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300 focus:border-indigo-500 dark:focus:border-indigo-600 focus:ring-indigo-500 dark:focus:ring-indigo-600" required>
                                        <option value="1" {{ old('is_active', $leaveType->is_active) == 1 ? 'selected' : '' }}>Oui</option>
                                        <option value="0" {{ old('is_active', $leaveType->is_active) == 0 ? 'selected' : '' }}>Non</option>
                                    </select>
                             ...
                             <div>
                                ...
                                <div class="flex items-center mt-1">
                                 ...
                                 {{-- Le script JS pour lier les champs est le même que dans create --}}
                                 <script> document.addEventListener('DOMContentLoaded', function() { const cp=document.getElementById('color_code'), ct=document.getElementById('color_code_text'); if(cp && ct) { cp.addEventListener('input', (e) => ct.value=e.target.value); ct.addEventListener('input', (e) => { if(/^#([A-Fa-f0-9]{6}|[A-Fa-f0-9]{3})$/i.test(e.target.value)) cp.value=e.target.value; }); } }); </script>
                            </div>
