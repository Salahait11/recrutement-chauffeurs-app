...
<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 dark:text-gray-200 leading-tight">
            {{ __('Créer un Nouveau Critère d\'Évaluation') }}
        </h2>
    </x-slot>
        ...
        <div class="max-w-4xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900 dark:text-gray-100">
                    ...
                    <form method="POST" action="{{ route('admin.evaluation-criteria.store') }}">
                        @csrf
                        <div class="space-y-6">
                            {{-- Nom --}}
                            <div>
                                <label for="name" class="block font-medium text-sm ...">{{ __('Nom du Critère') }} <span class="text-red-500">*</span></label>
                                <input type="text" name="name" id="name" class="block mt-1 w-full rounded-md ..." value="{{ old('name', $criterion->name ?? '') }}" required autofocus>
                            ...
                            <div>
                                ...
                                <select name="is_active" id="is_active" class="block mt-1 w-full rounded-md ..." required>
                                    <option value="1" {{ old('is_active', $criterion->is_active ?? true) == 1 ? 'selected' : '' }}>Oui</option>
                                    <option value="0" {{ old('is_active', $criterion->is_active ?? true) == 0 ? 'selected' : '' }}>Non</option>
                                </select>
