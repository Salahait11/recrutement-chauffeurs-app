    ...
    <x-slot name="header">
        <div class="flex justify-between items-center">
            <h2 class="font-semibold text-xl text-gray-800 dark:text-gray-200 leading-tight">
                {{ __('Modifier le Candidat') }} : {{ $candidate->getFullName() }}
            </h2>
            <div class="flex space-x-2">
                <a href="{{ route('candidates.show', $candidate->id) }}" class="inline-flex items-center px-4 py-2 bg-indigo-600 text-white rounded hover:bg-indigo-700">
        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900 dark:text-gray-100">
                    ...
                    <form method="POST" action="{{ route('candidates.update', $candidate->id) }}" class="space-y-6">
                        @csrf
                        @method('PUT')
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <div>
                                ...
                                <input type="date" name="birth_date" id="birth_date" 
                                       class="mt-1 block w-full rounded-md border-gray-300 shadow-sm dark:bg-gray-900 dark:border-gray-700 dark:text-gray-300 focus:border-indigo-500 focus:ring-indigo-500"
                                       value="{{ old('birth_date', $candidate->birth_date->format('Y-m-d')) }}" required>
                            </div>
                            ...
                            <div>
                                ...
                                <input type="text" name="driving_license_number" id="driving_license_number" 
                                       class="mt-1 block w-full rounded-md border-gray-300 shadow-sm dark:bg-gray-900 dark:border-gray-700 dark:text-gray-300 focus:border-indigo-500 focus:ring-indigo-500"
                                       value="{{ old('driving_license_number', $candidate->driving_license_number) }}" required>
                            </div>
                            ...
                            <div>
                                ...
                                <input type="date" name="driving_license_expiry" id="driving_license_expiry" 
                                       class="mt-1 block w-full rounded-md border-gray-300 shadow-sm dark:bg-gray-900 dark:border-gray-700 dark:text-gray-300 focus:border-indigo-500 focus:ring-indigo-500"
                                       value="{{ old('driving_license_expiry', $candidate->driving_license_expiry->format('Y-m-d')) }}" required>
                            </div>
                        </div>
                        ...
                        <div>
                            ...
                            <select name="status" id="status" 
                                ...
                                @foreach($statuses as $key => $value)
                                    <option value="{{ $key }}" {{ old('status', $candidate->status) == $key ? 'selected' : '' }}>
                                        {{ $value }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div>
                            ...
                            <textarea name="notes" id="notes" rows="4" 
                                      class="mt-1 block w-full rounded-md border-gray-300 shadow-sm dark:bg-gray-900 dark:border-gray-700 dark:text-gray-300 focus:border-indigo-500 focus:ring-indigo-500">{{ old('notes', $candidate->notes) }}</textarea>
                        </div>

                        <!-- Boutons -->
                        <div class="flex justify-between pt-4">
                            <a href="{{ route('candidates.show', $candidate->id) }}" class="inline-flex items-center px-4 py-2 bg-gray-200 dark:bg-gray-700 text-gray-800 dark:text-gray-200 rounded hover:bg-gray-300 dark:hover:bg-gray-600 transition">
                                <svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
